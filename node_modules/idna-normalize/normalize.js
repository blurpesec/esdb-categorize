...
class IDNANormalizer {
  normalize (domain) {
    var newstring = '';
    var lookup = ['a','e','i','o','u','l','w','.','h','m','t','y','b','c','d','f','g','j','k','n','p','q','r','s','v','x','z'];
    for ( var i = 0; i < domain.length; i++ ) {
      for ( var c = 0; c < lookup.length; c++ ) {
        if ( idnadict[lookup[c]].indexOf(domain.charAt(i)) > -1 ) {
          newstring += lookup[c];
          break;
        }
        if ( c == (lookup.length-1) ) {
          newstring += domain.charAt(i);
        }
      }
    }
    return newstring;
